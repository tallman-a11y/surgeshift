...
import { Zap, LayoutDashboard, Briefcase, Settings, LogOut, ScanSearch, TrendingUp, Menu, X } from 'lucide-react'
...
export default function Sidebar() {
        ...
        {user && (
          <div className="px-3 pb-4 border-t pt-3" style={{ borderColor: 'var(--color-border)' }}>
            <div className="flex items-center gap-2.5 px-2">
              ...
              <button type="button" onClick={handleSignOut} className="p-1.5 rounded-md transition-all" style={{ color: 'var(--color-text-muted)' }} title="Sign out">
                <LogOut size={13} />
              </button>
            </div>
          </div>
  ...
  return (
    <>
      ...
      >
        <button type="button" onClick={() => setMobileOpen(false)} aria-label="Close menu" title="Close menu" className="absolute top-4 right-4 p-1.5 rounded-md" style={{ color: 'var(--color-text-muted)' }}>
          <X size={16} />
        </button>
        <NavContent />
      </aside>
